...
def drive(cfg, use_joystick=False, camera_type='single'):
    ...
    #
    # survey position logger (added 2026-08-31)
    # Logs pos/x,pos/y in ALL modes so autonomous laps produce a GPS track.
    # The stock NMEA CsvLogger only records in user mode, so autopilot laps
    # were writing nothing. Wrapped in try/except: logging must never stop
    # the car.
    #
    if getattr(cfg, "HAVE_GPS", False):
        try:
            import os as _os, time as _time
            from survey_gps_logger import SurveyGpsLogger
            _gps_log = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)),
                                     "logs", "gps_%s.csv" % _time.strftime("%Y%m%d_%H%M%S"))
            V.add(SurveyGpsLogger(_gps_log),
                  inputs=["pos/x", "pos/y", "gps/nmea"])
        except Exception as _e:
            print("[survey_gps] logger not added:", _e)
