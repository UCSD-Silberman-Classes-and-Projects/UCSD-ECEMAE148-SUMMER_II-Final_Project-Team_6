...
# ============================================================
# TEAM 6 GPS LAPS CONFIG — added 2026-08-13
# Source: gps_laps.html §5, adapted to this robot's actual hardware
# ============================================================

# --- A. GPS -------------------------------------------------
# /dev/ttyUSB0 is the CP2105 "Enhanced" port and is where GGA
# sentences were confirmed streaming. The runner (Terminal 1)
# uses the OTHER port, /dev/ttyUSB1.
# GPS_SERIAL = "/dev/ttyUSB0"   # DEAD: overridden at line 737 with ttyUSB1. The RTK runner owns ttyUSB0.
GPS_SERIAL_BAUDRATE = 460800   # fixed rate of the PointOneNav; do NOT change
GPS_DEBUG = True               # prints coords; set False once working
HAVE_GPS = True                # master switch
GPS_NMEA_PATH = None           # None = live GPS (not file replay)

# --- B. VESC drivetrain ------------------------------------
DRIVE_TRAIN_TYPE = "VESC"
VESC_MAX_SPEED_PERCENT = .4    # matches the DL car setup
VESC_SERIAL_PORT = "/dev/ttyACM0"
VESC_BAUDRATE = 115200
VESC_STEERING_SCALE = 0.5      # with offset 0.5 this is already FULL servo range
VESC_STEERING_OFFSET = 0.5     # +/-0.02 if it drifts on a straight

# --- C. Joystick -------------------------------------------
USE_JOYSTICK_AS_DEFAULT = True
JOYSTICK_MAX_THROTTLE = 0.5
CONTROLLER_TYPE = 'ps4'       # guide says 'ps4' - WRONG for our hardware
JOYSTICK_DEVICE_FILE = "/dev/input/js1"

# --- D. Button mappings ------------------------------------
SAVE_PATH_BTN = "options"           # write waypoints to CSV
LOAD_PATH_BTN = "X"            # load saved path
RESET_ORIGIN_BTN = "B"         # set current position as (0,0)
ERASE_PATH_BTN = "Y"           # clear waypoints without saving
TOGGLE_RECORDING_BTN = "share"    # start/stop recording
INC_PID_D_BTN = None
DEC_PID_D_BTN = None
INC_PID_P_BTN = None
DEC_PID_P_BTN = None
...
# --- E. Camera ---------------------------------------------
CAMERA_TYPE = "MOCK"           # GPS laps need no camera

# --- F. PID (guide values; template defaults were NEGATIVE) -
PID_P = 0.15
PID_D = 0.6                    # rule of thumb: D ~ 4x P
PID_I = 0.0

# --- G. VESC fields required by complete.py add_drivetrain ---
# The path_follow template's config.py omits these; copied from ~/mycar.
